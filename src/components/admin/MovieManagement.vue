<template>
  ...
  <button v-if="currentTab === 'movies'" @click="changePage(page + 1)" :disabled="page === totalMoviePages">Sau</button>
  ...
  <button v-if="currentTab === 'series'" @click="changePage(page + 1)" :disabled="page === totalSeriesPages">Sau</button>
</div>
    <!-- Form Thêm/Sửa Phim -->
  ...
  methods: {
    ...
    async fetchSeries(page = 1) {
      ...
      try {
        ...
        // Calculate total pages based on items per page
        this.totalPages = Math.ceil(this.allSeries.length / this.itemsPerPage);
