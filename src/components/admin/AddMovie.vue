                ...
                <table class="data-table">
                    ...
                    <tbody>
                        ...
                        <tr v-if="paginatedMovies.length === 0 && !loading">
                            <td colspan="4">Không có phim nào phù hợp!</td>
                        </tr>
...
export default {
    data() {
        return {
            ...
            allMovies: [], // To store all movies fetched from API
            paginatedMovies: [], // To store the current page's movies
    ...
    methods: {
        async fetchMovies() {
            ...
            try {
                const response = await axios.get("http://localhost:5289/api/AdminMovie", {
                    params: {
                        sortBy: "Title",
                        search: this.searchQuery.trim(), // Từ khóa tìm kiếm
                    },
                });

                console.log("Dữ liệu trả về từ API:", response.data);

                // Lưu toàn bộ dữ liệu phim
                ...
                // Gọi hàm để cập nhật dữ liệu phân trang cho trang hiện tại
        ...
        async searchMovies() {
            this.page = 1; // Quay về trang đầu tiên khi tìm kiếm
            await this.fetchMovies(); // Gọi API với từ khóa tìm kiếm
        },
        changePage(newPage) {
        ...
        selectTab(tab) {
            ...
            this.page = 1; // Đặt lại trang đầu tiên
            if (tab === "movies") {
                this.fetchMovies();
            } else {
                // Tương tự logic fetchSeries() nếu cần
                console.log("Fetching series...");
            }
        },
    },
    mounted() {
        this.fetchMovies(); // Lấy danh sách phim lẻ khi tải trang
    },
};
</script>
